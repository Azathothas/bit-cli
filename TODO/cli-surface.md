...
| --- | --- | --- |
| `every_short_flag_is_documented_in_the_flags_table` | `cli.rs:2361` | a short flag with no row in `docs/flags.md` |
| `no_short_flag_is_defined_twice` | `cli.rs:2132` | one letter used twice in one command |
| `short_flags_never_contradict_aria2` | `cli.rs:2168` | an `aria2` letter reassigned to a different concept |
| `short_flags_keep_their_aria2_meanings` | `cli.rs:1953` | `-V` no longer meaning `--check-integrity` |
...
Acceptance:  Two parts, and the first is what stops this recurring.
             ...
             `cli.rs:2361` `every_short_flag_is_documented_in_the_flags_table`
...
Problem:     `scripts/check-todo.ps1` resolved a citation written long, as
             `crates/bit-cli/src/cli.rs:2132`, and checked only that the file
             had that many lines. Most of `TODO/` does not write them long. A
             citation written as `cli.rs:2132` matched nothing in the pattern,
