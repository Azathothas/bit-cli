...
Remaining:   1. `--version hybrid` is not covered because it does not exist.
             ...
             2. Only `aria2c` is wired up. `rqbit` and `transmission-cli` are
                not installed on this machine and installing them needs an
                operator, per PROMPT.md 0.4. The script takes `-Client`, and
                adding one is a new branch in the invocation block. Until then
                the claim is "a different client", not "every client".
