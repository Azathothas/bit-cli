...
> **Note on this copy.** Trimmed to BitTorrent-relevant material. Removed:
> `docs/MIGRATION.md`, `docs/compatibility-status.md`, `docs/testing-guide.md`
> (non-BitTorrent migration/compatibility bookkeeping), `bindings/` (C,
> Node.js and Python language bindings), the top-level `benches/` and
> `scripts/`, and the installation/quick-start/usage sections of this file.
> References to those paths below are left as upstream wrote them.
> Retained: `aria2-protocol/src/bittorrent/` (bencode, DHT, MSE, PEX,
> ut_metadata, fast_set, piece picker, trackers, uTP), the `aria2-core`
> `bt_*` engine modules and their tests, `aria2-rpc/`,
> `docs/comprehensive_gap_analysis.md` and
> `docs/performance-differentiators.md`.
