...
> **Note on this copy.** Trimmed for BitTorrent research: `res/` (app icons and
> 252 country-flag PNGs), `lang/` (translation JSON), `installer/`,
> `tools/update-flags.ps1`, `AI-DECLARATION.md` and the vendored librqbit WebUI
> (`vendor/librqbit/webui/`) were removed, along with the build section of this
> file. `build.rs` verifies the patch stack at compile time and the binary
> embeds the removed `lang/*.json`, so this copy is for reading, not building.
> Retained: `src/`, `patches/`, `vendor/`, `migrations/`,
