...
pub(crate) fn digit_range_transformed_to_hex_without_leading_zero() -> String {
    let mut rng = rand::rng();
    format!("{:x}", rng.random_range(1u32..2147483647u32)).to_uppercase()
}
...
pub(crate) fn random_pool_with_checksum(prefix: &str, characters_pool: &str) -> String {
    ...
    out.push_str(prefix);
    //TODO:
    let random_bytes = rand::rng().random::<[u8; PEER_ID_LENGTH]>();
    let base = characters_pool.chars().count();
    for b in random_bytes {
        out.push(
            characters_pool
                .chars()
                .nth(usize::from(b) % base)
                .expect("Cannot get char in characters pool"),
        );
        if out.len() == PEER_ID_LENGTH {
            break;
        }
    }
    out
