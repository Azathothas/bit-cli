...
impl Client {
    ...
    pub fn get_query(&self) -> (String, Vec<(String, String)>) {
        ...
        if !self.accept.is_empty() {
            headers.push((String::from("Accept"), self.accept.clone()));
        }
    ...
    pub fn generate_key(&mut self) {
        let key = match &self.key_algorithm {
            ...
            algorithm::Algorithm::DigitRangeTransformedToHexWithoutLeadingZeroes => {
                algorithm::digit_range_transformed_to_hex_without_leading_zero()
            }
            algorithm::Algorithm::Regex => byte_serialize(
                &algorithm::regex(self.key_pattern.clone()).as_bytes()[0..KEY_LENGTH],
            )
            .collect(),
    ...
    pub fn generate_peer_id(&mut self) {
        let hash = match &self.peer_algorithm {
            ...
            algorithm::Algorithm::RandomPoolWithChecksum => {
                algorithm::random_pool_with_checksum(&self.peer_prefix, &self.peer_pattern)
            }
            _ => String::new(),
        };
        self.peer_id = byte_serialize(&hash.as_bytes()[0..PEER_ID_LENGTH]).collect();
        //take the first 20 charsencode it because weird chars
    }
}

#[cfg(test)]
mod tests {
    use crate::{clients::ClientVersion, Client};

    const CLIENT_VERSIONS: [ClientVersion; 85] = [
