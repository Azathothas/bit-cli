...
> **Note on this copy.** Trimmed for BitTorrent research: the Wails desktop
> GUI (`gui/`), `docs/benchmarks/` (plot images and a matplotlib script),
> `.claude/`, `CLAUDE.md` and `Makefile` were removed, along with the
> badges, installation, GUI and table-of-contents sections of this file.
> Retained: `torrent/`, `cmd/`, `internal/`, `utils/`, `schema/`,
> `examples/`, `scripts/`.
