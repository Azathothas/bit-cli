...
3. patches/UPSTREAM.md  every change already made to somebody else's code, and
                        whether it has been offered upstream.
...
6. TODO/RULES.md        how this repository is worked on, including the only
...
- Every vendored change gets a section in patches/UPSTREAM.md BEFORE the
  ...
  done outside the vendored tree, and whether it is meant to go upstream. A
  change with no entry behind it says so explicitly rather than inventing one.
