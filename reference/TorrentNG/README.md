...
> **Note on this copy.** This tree was trimmed for BitTorrent research. The
> React WebUI (`webui/`), the Docker/systemd/nginx deployment trees
> (`deploy/docker`, `deploy/native`, `deploy/nginx`, `deploy/systemd`),
> `packaging/`, `certification/`, `engine-profile/` and `CONTRIBUTING.md`
> were removed. Documents under `docs/` still reference those paths; the
> retained material is `crates/`, `sidecar/`, `docs/`, `scripts/`,
> `benchmarks/`, `testdata/`, `deploy/interop/`, `deploy/certification/`
> and `torrentng_engine_rewrite_spec.md`.
